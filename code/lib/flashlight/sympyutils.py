...
def subs_matrix_verbose(A_expr,subs,simultaneous=False):
    ...
    print "flashlight.sympyutils: subs_matrix_verbose(...) begin..."
    ...
    print "flashlight.sympyutils: subs_matrix_verbose(...) end."
    return A_subs_expr
...
def generate_c_code(expr,syms,func_name,tmp_dir,out_dir,verbose=False,request_delete_tmp_dir=True):
    ...
    if verbose: print "flashlight.sympyutils: Generating Cython code for %s..." % func_name
    _autowrap_skip_compile(expr=expr,backend="cython",verbose=verbose,args=syms,tempdir=tmp_dir)
    if verbose: print "flashlight.sympyutils: Modifying autogenerated Cython files and building..."
...
def generate_c_code_cse(expr,syms,func_name,tmp_dir,out_dir,cse_ordering="canonical",verbose=False,request_delete_tmp_dir=True):
    ...
    if verbose: print "flashlight.sympyutils: Performing common subexpression elimination..."
    subexpr_list,expr_in_terms_of_subexprs = sympy.cse(expr,order=cse_ordering)
    if verbose: print "flashlight.sympyutils: Finished performing common subexpression elimination."
...
def build_module_autowrap(expr,syms,module_name,tmp_dir,out_dir,dummify=False,cse=False,cse_ordering="canonical",build_vectorized=False,pretty_dummy_symbol_names=False,verbose=False,request_delete_tmp_dir=True):
    ...
    if dummify:
        if verbose: print "flashlight.sympyutils: Generating dummy symbols for %s..." % module_name        
        expr,syms = _dummify(expr,syms,pretty_dummy_symbol_names)
        if verbose: print "flashlight.sympyutils: Finished generating dummy symbols for %s." % module_name
    ...
    if verbose: print "flashlight.sympyutils: Generating Cython code for %s..." % module_name
    _autowrap_skip_compile(expr=expr,backend="cython",verbose=verbose,args=syms,tempdir=tmp_dir)
    if verbose: print "flashlight.sympyutils: Modifying autogenerated Cython files for %s" % module_name
    ...
    if build_vectorized:
        if verbose: print "flashlight.sympyutils: Generating Cython code for %s_vectorized..." % module_name
        if is_matrix_expr:
    ...
    try:
        ...
        if verbose: print "flashlight.sympyutils: Building Cython code for %s..." % module_name
        ...
        if build_vectorized:
            if verbose: print "flashlight.sympyutils: Building Cython code for %s_vectorized..." % module_name
